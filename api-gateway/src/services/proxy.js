...
const createServiceProxy = (serviceName, serviceUrl, options = {}) => {
  return createProxyMiddleware({
    ...
    onProxyReq: (proxyReq, req, res) => {
      ...
      console.log(
        `[${serviceName.toUpperCase()}] ${req.method} ${req.originalUrl}`
      );
      console.log(
        `[${serviceName.toUpperCase()}] Target URL: ${serviceUrl}${req.url}`
      );

      // QUAN TRỌNG: Luôn chuyển tiếp Authorization header nguyên vẹn
      if (req.headers.authorization) {
        console.log(
          `[${serviceName.toUpperCase()}] Forwarding Authorization header`
        );
      ...
      if (req.user) {
        if (req.user.user_id || req.user.id) {
          proxyReq.setHeader(
            "X-User-ID",
            (req.user.user_id || req.user.id).toString()
          );
          console.log(
            `[${serviceName.toUpperCase()}] Setting X-User-ID: ${
        ...
        if (req.user.role) {
          ...
          console.log(
            `[${serviceName.toUpperCase()}] Setting X-User-Role: ${
              req.user.role
            }`
          );
        }
        ...
        console.log(
          `[${serviceName.toUpperCase()}] User info:`,
    ...
    onProxyRes: (proxyRes, req, res) => {
      // Log response status
      console.log(
        `[${serviceName.toUpperCase()}] Response Status: ${proxyRes.statusCode}`
      );
    },
    onError: (err, req, res) => {
      // Xử lý lỗi proxy
...
const authServiceProxy = createProxyMiddleware({
  ...
  onProxyRes: (proxyRes, req, res) => {
    ...
    // For authentication endpoints, capture and log the response body for debugging
    if (req.path.includes("/login") || req.path.includes("/token")) {
      let responseBody = "";
      ...
      proxyRes.on("end", function () {
        try {
          ...
          console.log(
            `[AUTH] Response Body (sanitized): ${JSON.stringify({
              ...parsedBody,
              access: parsedBody.access ? "***TOKEN***" : undefined,
              refresh: parsedBody.refresh ? "***TOKEN***" : undefined,
          ...
          console.log(
            "[AUTH] Could not parse response body:",
  ...
  onError: (err, req, res) => {
    ...
    console.error("[AUTH] Error details:", JSON.stringify(errorDetails));

    res.status(500).json({
      status: "error",
      message: `Auth service is unavailable: ${err.message}`,
      error: errorDetails,
    });
  },
});

const appointmentServiceProxy = createServiceProxy(
  "appointments",
  config.services.appointment
);
const medicalRecordServiceProxy = createServiceProxy(
  "medical-records",
  config.services.medicalRecord
);
const billingServiceProxy = createServiceProxy(
  "billing",
...
const pharmacyServiceProxy = createServiceProxy(
  "pharmacy",
  config.services.pharmacy
);
