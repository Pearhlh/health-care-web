...
router.use("/api/auth/token", authServiceProxy);
...
router.get("/api/users/doctors/:doctor_id", (req, res, next) => {
  console.log(
    `[USER] Forwarding doctor info request for doctor_id: ${req.params.doctor_id}`
  );
  ...
  console.log(
    `[USER] Redirecting from /api/users/doctors/${doctorId} to ${req.url}`
  );
...
router.get("/api/appointments/test", (req, res, next) => {
  ...
  axios
    .get(`${appointmentServiceUrl}/api/test/`, { headers })
    .then((response) => {
...
router.get("/api/direct/patient-appointments", (req, res) => {
  ...
  console.log(
    "Sending request to:",
    `${appointmentServiceUrl}/api/appointments/patient-appointments/`
  );
  console.log("With headers:", headers);

  // Call the service directly
  axios
    .get(`${appointmentServiceUrl}/api/appointments/patient-appointments/`, {
      headers,
    })
    .then((response) => {
    ...
    .catch((error) => {
      console.error(
        "Error calling patient-appointments endpoint:",
        error.message
      );
      if (error.response) {
...
router.post(
  "/api/appointments/:appointment_id/create-encounter",
  (req, res, next) => {
...
router.use("/api/pharmacy", (req, res, next) => {
  ...
  if (req.user && req.user.role) {
    console.log(
      `[PHARMACY] Setting role ${req.user.role} for user_id ${req.user.user_id}`
    );
...
router.use("/api/invoices", (req, res, next) => {
  ...
  if (req.user && req.user.role) {
    console.log(
      `[BILLING] Setting role ${req.user.role} for user_id ${req.user.user_id}`
    );
    req.headers["X-User-Role"] = req.user.role;
  }
...
router.use("/api/insurance-claims", (req, res, next) => {
  ...
  if (req.user) {
    ...
    req.headers["X-User-Last-Name"] = req.user.last_name || "";
    console.log(
      `[BILLING] Insurance claim request from: ${JSON.stringify(req.user)}`
    );
  }
...
router.use("/api/laboratory", (req, res, next) => {
  ...
  if (req.user && req.user.role) {
    console.log(
      `[LABORATORY] Setting role ${req.user.role} for user_id ${req.user.user_id}`
    );
    req.headers["X-User-Role"] = req.user.role;
  }
...
router.use("/api/notifications", (req, res, next) => {
  ...
  if (req.user && req.user.role) {
    console.log(
      `[NOTIFICATION] Setting role ${req.user.role} for user_id ${req.user.user_id}`
    );
    req.headers["X-User-Role"] = req.user.role;
  }
...
router.use("/api/in-app-notifications", (req, res, next) => {
  ...
  if (req.user) {
    ...
    req.headers["X-User-Last-Name"] = req.user.last_name || "";
    console.log(
      `[NOTIFICATION] In-app notification request from: ${JSON.stringify(
        req.user
      )}`
    );
  }
  ...
  console.log(
    `[NOTIFICATION] Sending request to: ${notificationServiceUrl}/api/in-app/`
  );

  // Call the service directly
  axios
    .get(`${notificationServiceUrl}/api/in-app/`, { headers })
    .then((response) => {
    ...
    .catch((error) => {
      console.error(
        "[NOTIFICATION] Error calling in-app notifications:",
        error.message
      );
      if (error.response) {
        console.error(
...
router.post("/api/notifications/events", (req, res, next) => {
  ...
  req.url = "/api/events";
  console.log(
    `[NOTIFICATIONS] Target URL: http://notification-service:8006${req.url}`
  );
