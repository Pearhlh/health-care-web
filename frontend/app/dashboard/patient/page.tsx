...
export default function PatientDashboardPage() {
  ...
  useEffect(() => {
    const fetchMeta = async () => {
      try {
        // Giả sử có API lấy danh sách khoa và chuyên khoa
        ...
        toast.error("Không thể tải danh sách khoa/chuyên khoa");
      }
    };
    fetchMeta();
  }, []);
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      ...
      try {
        ...
        // Gọi API lấy danh sách bác sĩ phù hợp
    ...
    fetchDoctors();
  }, [selectedDepartment, selectedSpecialization, selectedDate]);
    ...
    try {
      ...
      const formattedAppointments = appointmentsResponse.map((appointment) => {
        ...
        // Nếu không có doctor, thử sử dụng doctor_info
          ...
          typeof appointment.doctor_info === "object"
        ) {
          doctorInfo = {
            ...doctorInfo,
      ...
      try {
        const prescriptionsResponse =
          await PharmacyService.getPatientPrescriptions();
        console.log("Prescriptions data from API:", prescriptionsResponse);

        // Chuyển đổi dữ liệu từ API sang định dạng hiển thị
        formattedPrescriptions = await Promise.all(
          prescriptionsResponse.map(async (prescription) => {
            ...
            try {
              if (prescription.doctor_id) {
                const doctorResponse = await MedicalRecordService.getDoctorInfo(
                ...
                if (doctorResponse) {
                  doctorInfo = {
                    ...
                    specialty:
                      doctorResponse.doctor_profile?.specialty?.name || "",
                  };
                }
              }
            } catch (error) {
              console.error(
                `Error fetching doctor info for prescription ${prescription.id}:`,
                error
              );
            }
            ...
            return {
              ...
              doctor: doctorInfo,
              prescription_date:
                prescription.date_prescribed || prescription.created_at,
      ...
      try {
        ...
        // Chuyển đổi dữ liệu từ API sang định dạng hiển thị
        formattedLabTests = await Promise.all(
          labTestsResponse.map(async (test) => {
            // Lấy thông tin bác sĩ nếu cần
            let doctorInfo = {
              first_name: "",
              last_name: "",
            ...
            try {
              ...
              console.error(
                `Error fetching doctor info for lab test ${test.id}:`,
                error
              );
            }
            ...
            return {
              id: test.id,
              test_name:
                test.test_type_details?.name || "Xét nghiệm không xác định",
      ...
      // Cập nhật thống kê
      setStats({
        upcomingAppointments: formattedAppointments.filter(
          (a) => a.status === "CONFIRMED" || a.status === "SCHEDULED"
        ).length,
        pendingPrescriptions: formattedPrescriptions.filter(
          (p) => p.status === "PENDING"
        ).length,
        pendingLabTests: formattedLabTests.filter((l) => l.status === "PENDING")
          .length,
  ...
  const handleBook = async () => {
    ...
    try {
      await appointmentService.createAppointment({
        ...
        time_slot_id: selectedTimeSlot ? Number(selectedTimeSlot) : undefined,
        reason_text: reason,
    ...
    <PageContainer>
      <PageHeader
        ...
        actions={
          <Button
            onClick={() =>
              router.push("/dashboard/patient/appointments/simple-booking")
            }
            className='group'>
            ...
            <Card>
              <CardHeader>
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <CardDescription>
                      Danh sách các cuộc hẹn sắp tới của bạn
                    </CardDescription>
                  </div>
                  <Button
                    variant='outline'
              ...
              <CardFooter className='flex justify-center border-t pt-4'>
                <Button
                  onClick={() =>
                    router.push(
                      "/dashboard/patient/appointments/simple-booking"
                    )
                  }
                  className='group'>
                  <PlusCircle className='mr-2 h-4 w-4 transition-transform group-hover:rotate-90' />
              ...
              <CardHeader>
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <CardDescription>
                      Danh sách đơn thuốc gần đây của bạn
                    </CardDescription>
                  </div>
                  <Button
                    variant='outline'
                      ...
                      <motion.div
                        ...
                        <div>
                          ...
                          <p className='text-sm text-muted-foreground'>
                            BS. {prescription.doctor.first_name}{" "}
              ...
              <CardHeader>
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <CardDescription>
                      Danh sách xét nghiệm gần đây của bạn
                    </CardDescription>
                  </div>
                  <Button
                    variant='outline'
                    ...
                    {labTests.map((test, index) => (
                      <motion.div
                        ...
                        <div>
                          ...
                          <p className='text-sm text-muted-foreground'>
                            BS. {test.doctor.first_name} {test.doctor.last_name}{" "}
                        ...
                        <div className='mt-4 flex items-center gap-2 md:mt-0'>
                          <Button
                            ...
                            onClick={() =>
                              router.push(
                                `/dashboard/patient/records/lab-tests/${test.id}`
                              )
                            }
                            className='group'>
                            <FileText className='mr-2 h-3.5 w-3.5' />
      ...
      <div className='max-w-xl mx-auto py-8'>
        <Card>
          <CardHeader>
            <CardTitle>Đặt lịch hẹn khám bệnh</CardTitle>
          </CardHeader>
          <CardContent className='space-y-4'>
              ...
              <Select
                ...
                <SelectContent>
                  {Array.isArray(departments) &&
                    departments.map((dep) => (
                      <SelectItem
                        key={dep.id || dep.code || dep.name}
                        value={dep.code || dep.id || dep.name}>
                        {dep.name}
                      </SelectItem>
                    ))}
                </SelectContent>
            ...
            <div>
              <label className='block mb-1 font-medium'>Chọn chuyên khoa</label>
              <Select
                ...
                <SelectContent>
                  {specializations.map((spec) => (
                    <SelectItem
                      key={spec.id || spec.code || spec.name}
                      value={spec.code || spec.id || spec.name}>
                      {spec.name}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div>
              <label className='block mb-1 font-medium'>Chọn khung giờ</label>
              <Select
                ...
                <SelectTrigger>
                  <SelectValue
                    placeholder={isLoading ? "Đang tải..." : "Chọn khung giờ"}
                  />
                </SelectTrigger>
                <SelectContent>
                  {timeSlots.length === 0 ? (
